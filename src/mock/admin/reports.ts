import type { DistributionLog, Metric, PackageSegment, SettlementLog } from '../../features/admin/reports/types';

const ADMIN_REPORT_SETTLEMENT_LOGS: SettlementLog[] = [
...
const ADMIN_REPORT_METRICS: Metric[] = [
...
const ADMIN_REPORT_PACKAGE_SEGMENTS: PackageSegment[] = [
...
const ADMIN_REPORT_DISTRIBUTION_LOGS: DistributionLog[] = [
...
export const getInitialAdminReportData = () => ({
  settlementLogs: ADMIN_REPORT_SETTLEMENT_LOGS.map((log) => ({ ...log })),
  metrics: ADMIN_REPORT_METRICS.map((metric) => ({ ...metric })),
  packageSegments: ADMIN_REPORT_PACKAGE_SEGMENTS.map((segment) => ({ ...segment })),
  distributionLogs: ADMIN_REPORT_DISTRIBUTION_LOGS.map((log) => ({ ...log }))
});
