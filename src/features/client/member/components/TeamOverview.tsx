...
import type { RecentActivity, TeamOverviewProps } from '../types';
...
export default function TeamOverview({ recentActivities }: TeamOverviewProps) {
  const columns: TableColumnsType<ReturnType<typeof RecentActivityDesktopRow>> = [
    { title: '用户', dataIndex: 'user', key: 'user' },
    { title: '动作', dataIndex: 'action', key: 'action' },
    { title: '时间', dataIndex: 'time', key: 'time', align: 'right' },
  ];

  const tableData = recentActivities.map((activity: RecentActivity) => RecentActivityDesktopRow({ activity }));
    ...
    <div className="lg:col-span-8 glass-card rounded-2xl p-6 md:p-8 flex flex-col gap-6">
        ...
        <div className="md:col-span-8 flex flex-col sm:flex-row items-center justify-around bg-[#211f24]/30 rounded-2xl p-6 border border-white/5">
          <div className="relative w-36 h-36">
            <svg className="w-full h-full -rotate-90" viewBox="0 0 144 144" width="144" height="144">
              <circle className="text-[#36343a]" cx="72" cy="72" fill="transparent" r="58" stroke="currentColor" strokeWidth="10" />
              <circle className="text-[#cfbcff]" cx="72" cy="72" fill="transparent" r="58" stroke="currentColor" strokeDasharray="364.4" strokeDashoffset="236.8" strokeLinecap="round" strokeWidth="10" />
              <circle className="text-[#cdc0e9]" cx="72" cy="72" fill="transparent" r="58" stroke="currentColor" strokeDasharray="364.4" strokeDashoffset="310" strokeLinecap="round" strokeWidth="10" style={{ transform: 'rotate(126deg)', transformOrigin: 'center' }} />
            </svg>
          ...
          <div className="space-y-3 mt-4 sm:mt-0">
            <div className="flex items-center gap-3">
              <span className="w-2.5 h-2.5 rounded-full bg-[#cfbcff] glow-accent" />
            ...
            <div className="flex items-center gap-3">
              <span className="w-2.5 h-2.5 rounded-full bg-[#cdc0e9] glow-accent" />
              <div className="flex flex-col">
      ...
      <div className="space-y-3 pt-4 border-t border-white/5">
        <h3 className="text-xs font-bold uppercase tracking-wider text-[#cbc4d2] mb-1 flex items-center gap-1">
          <TrendingUp className="w-4 h-4 text-[#cfbcff]" /> 最近下线动态
        </h3>
