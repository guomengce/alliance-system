...
export default function PurchaseHistory({
  ...
  const columns: TableColumnsType<Purchase> = [
      ...
      render: (id: string) => {
        ...
        return (
          <div className="flex items-center gap-1.5">
            <span className="font-mono font-bold text-white/90 select-all">{id}</span>
            <Button
    ...
    },
    {
      title: '方案产品',
      dataIndex: 'name',
      render: (name: string) => <span className="font-bold text-white">{name}理财</span>
    },
    {
      title: '金额 (USDT)',
      dataIndex: 'amount',
      ...
      render: (amount: number) => (
        <span className="font-bold text-white font-mono">{amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
      )
    },
    {
      title: '赠送比例',
      dataIndex: 'giftRatio',
      align: 'right',
      render: (giftRatio: number) => <span className="font-medium text-[#cfbcff] font-mono">{giftRatio}%</span>
    },
    {
      title: '预计 TROO',
      dataIndex: 'troo',
      align: 'right',
      render: (troo: number) => <span className="font-bold text-emerald-400 font-mono">{troo.toLocaleString()}</span>
    },
    {
      title: '日期时间',
      dataIndex: 'date',
